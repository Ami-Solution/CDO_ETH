...
export class ZeroX_ExchangeContract extends BaseContract {
    ...
    > {
            const self = this as ZeroX_ExchangeContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ZeroX_ExchangeContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ZeroX_ExchangeContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ZeroX_ExchangeContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ZeroX_Exchange.json", "utf8");

...
export class MockContractContract extends BaseContract {
    ...
    > {
            const self = this as MockContractContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<MockContractContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new MockContractContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<MockContractContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/MockContract.json", "utf8");

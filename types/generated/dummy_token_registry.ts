...
export class DummyTokenRegistryContract extends BaseContract {
    public setTokenAddress = {
        async sendTransactionAsync(
            ...
            const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
                txData,
                self.setTokenAddress.estimateGasAsync.bind(
                    self,
                    symbol,
                    token,
                ),
            ...
            const txHash = await promisify<string>(
                self.web3ContractInstance.setTokenAddress, self.web3ContractInstance,
            )(
                symbol,
                token,
                txDataWithDefaults,
        ...
        async estimateGasAsync(
            ...
            const gas = await promisify<number>(
                self.web3ContractInstance.setTokenAddress.estimateGas, self.web3ContractInstance,
            )(
                symbol,
                token,
                txDataWithDefaults,
        ...
        async getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = await promisify<string>(
                self.web3ContractInstance.setTokenAddress.getData, self.web3ContractInstance,
            )(
                symbol,
                token,
                txDataWithDefaults,
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<DummyTokenRegistryContract> {
    ...
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<DummyTokenRegistryContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/DummyTokenRegistry.json", "utf8");

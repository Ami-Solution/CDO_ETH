...
export class NonFungibleTokenContract extends BaseContract {
    public name = {
        async callAsync(
            defaultBlock?: Web3.BlockParam,
        ): Promise<string
    > {
            const self = this as NonFungibleTokenContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<NonFungibleTokenContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new NonFungibleTokenContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<NonFungibleTokenContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/NonFungibleToken.json", "utf8");

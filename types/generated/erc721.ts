...
export class ERC721Contract extends BaseContract {
    ...
    > {
            const self = this as ERC721Contract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ERC721Contract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ERC721Contract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ERC721Contract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ERC721.json", "utf8");

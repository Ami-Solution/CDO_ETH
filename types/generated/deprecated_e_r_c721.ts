...
export class DeprecatedERC721Contract extends BaseContract {
    public name = {
        async callAsync(
            defaultBlock?: Web3.BlockParam,
        ): Promise<string
    > {
            const self = this as DeprecatedERC721Contract;
    ...
    > {
            ...
            const result = await promisify<BigNumber[]
    >(
                self.web3ContractInstance.tokensOf.call,
                self.web3ContractInstance,
            )(
                _owner,
            );
    ...
    public takeOwnership = {
        async sendTransactionAsync(
            ...
            const txDataWithDefaults = await self.applyDefaultsToTxDataAsync(
                txData,
                self.takeOwnership.estimateGasAsync.bind(
                    self,
                    _tokenId,
                ),
            );
            const txHash = await promisify<string>(
                self.web3ContractInstance.takeOwnership, self.web3ContractInstance,
            )(
                _tokenId,
                txDataWithDefaults,
            );
        ...
        async estimateGasAsync(
            ...
            const gas = await promisify<number>(
                self.web3ContractInstance.takeOwnership.estimateGas, self.web3ContractInstance,
            )(
                _tokenId,
                txDataWithDefaults,
            );
        ...
        async getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = await promisify<string>(
                self.web3ContractInstance.takeOwnership.getData, self.web3ContractInstance,
            )(
                _tokenId,
                txDataWithDefaults,
            );
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<DeprecatedERC721Contract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new DeprecatedERC721Contract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<DeprecatedERC721Contract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/DeprecatedERC721.json", "utf8");

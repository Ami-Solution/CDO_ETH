...
export class ERC721BasicTokenContract extends BaseContract {
    public getApproved = {
        async callAsync(
            _tokenId: BigNumber,
    ...
    > {
            const self = this as ERC721BasicTokenContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ERC721BasicTokenContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ERC721BasicTokenContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ERC721BasicTokenContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ERC721BasicToken.json", "utf8");

...
export class TermsContractContract extends BaseContract {
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<TermsContractContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new TermsContractContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<TermsContractContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/TermsContract.json", "utf8");

...
export class CollateralizedSimpleInterestTermsContractContract extends BaseContract {
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<CollateralizedSimpleInterestTermsContractContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new CollateralizedSimpleInterestTermsContractContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<CollateralizedSimpleInterestTermsContractContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/CollateralizedSimpleInterestTermsContract.json", "utf8");

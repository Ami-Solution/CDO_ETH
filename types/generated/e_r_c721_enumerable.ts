...
export class ERC721EnumerableContract extends BaseContract {
    public getApproved = {
        async callAsync(
            _tokenId: BigNumber,
    ...
    > {
            const self = this as ERC721EnumerableContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ERC721EnumerableContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ERC721EnumerableContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ERC721EnumerableContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ERC721Enumerable.json", "utf8");

...
export class ZeroX_DummyTokenContract extends BaseContract {
    public name = {
        async callAsync(
            defaultBlock?: Web3.BlockParam,
        ): Promise<string
    > {
            const self = this as ZeroX_DummyTokenContract;
    ...
    public setBalance = {
        async sendTransactionAsync(
            ...
            const txHash = await promisify<string>(
                self.web3ContractInstance.setBalance, self.web3ContractInstance,
            )(
                _target,
                _value,
                txDataWithDefaults,
        ...
        async estimateGasAsync(
            ...
            const gas = await promisify<number>(
                self.web3ContractInstance.setBalance.estimateGas, self.web3ContractInstance,
            )(
                _target,
                _value,
                txDataWithDefaults,
        ...
        async getABIEncodedTransactionData(
            ...
            const abiEncodedTransactionData = await promisify<string>(
                self.web3ContractInstance.setBalance.getData, self.web3ContractInstance,
            )(
                _target,
                _value,
                txDataWithDefaults,
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ZeroX_DummyTokenContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ZeroX_DummyTokenContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ZeroX_DummyTokenContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ZeroX_DummyToken.json", "utf8");

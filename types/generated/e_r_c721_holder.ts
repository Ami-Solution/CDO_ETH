...
export class ERC721HolderContract extends BaseContract {
    public onERC721Received = {
        async sendTransactionAsync(
            ...
            const self = this as ERC721HolderContract;
    ...
    static async deployed(web3: Web3, defaults: Partial<TxData>): Promise<ERC721HolderContract> {
        const currentNetwork = web3.version.network;
        const { abi, networks } = await this.getArtifactsData(web3);
        const web3ContractInstance = web3.eth.contract(abi).at(networks[currentNetwork].address);

        return new ERC721HolderContract(web3ContractInstance, defaults);
    }
    static async at(address: string, web3: Web3, defaults: Partial<TxData>): Promise<ERC721HolderContract> {
    ...
    {
        try {
            const artifact = await fs.readFile("build/contracts/ERC721Holder.json", "utf8");
